...
posts_df = spark.read.format('xml').options(rowTag='row').options(rootTag='posts')\
                .load('s3://stackoverflow-events/06-24-2019/Posts.xml.lzo')
posts_df.write.mode('overwrite')\
        .parquet('s3://stackoverflow-events/06-24-2019/Posts.df.parquet')

users_df = spark.read.format('xml').options(rowTag='row').options(rootTag='users')\
                .load('s3://stackoverflow-events/06-24-2019/Users.xml.lzo')
users_df.write.mode('overwrite')\
        .parquet('s3://stackoverflow-events/06-24-2019/Users.df.parquet')
